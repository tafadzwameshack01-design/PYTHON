...
# League average total goals (realistic baseline)
...
# ============================================================
# IMPROVED MODEL (league-aware + home advantage)
# ============================================================
...
def add_scores(matches: List[Dict]) -> List[Dict]:
    out = []
    for m in matches:
        conf, goals, features = score_match(m['home_team'], m['away_team'], m['league'])
        m = dict(m)
...
def render_predictions_tab():
    with st.sidebar:
        ...
        min_conf = st.slider("Min Confidence (%)", 50, 93, 72, 1)
        include_live = st.checkbox("Include live games", value=True)
        league_filter = st.multiselect("Filter by League", options=list(LEAGUES.values()), default=[], placeholder="All leagues")
        st.markdown("---")
        st.info("📡 Data: API-Football (RapidAPI)\n\n🔄 Refreshes every 5 min\n\n✅ Model improved with league averages")
    ...
    if not all_matches:
        st.error("Could not fetch match data. Check RapidAPI key/quota.")
        return
    ...
    display = upcoming[:]
    if include_live: display += live
    if league_filter:
        display = [m for m in display if m['league'] in league_filter]

    st.subheader("📊 Today's Overview")
    c1, c2, c3, c4 = st.columns(4)
    with c1: st.markdown(f"<div class='stat-box'><h3>{len(upcoming)}</h3><p>Upcoming</p></div>", unsafe_allow_html=True)
    with c2: st.markdown(f"<div class='stat-box'><h3>{len(live)}</h3><p>Live Now 🔴</p></div>", unsafe_allow_html=True)
    with c3: st.markdown(f"<div class='stat-box'><h3>{len(finished)}</h3><p>Finished Today</p></div>", unsafe_allow_html=True)
    with c4: st.markdown(f"<div class='stat-box'><h3>{len(set(m['league'] for m in all_matches))}</h3><p>Leagues Active</p></div>", unsafe_allow_html=True)
    ...
    if not top:
        top = ranked[:5]
        st.warning(f"No games above {min_conf}% — showing top 5")
    ...
    for game in top[:15]:
        ...
        badge_color = "#00ff00" if conf >= 85 else "#ffcc00" if conf >= 75 else "#ff8800" if conf >= 60 else "#888888"
        ...
        with card_col:
            ...
            <div class='game-card'>
                <div style='display:flex;justify-content:space-between;align-items:center;'>
                    <div style='flex:1;'>
                        <span style='font-size:11px;color:#888;text-transform:uppercase;letter-spacing:1px;'>{game['league']} · {time_label}</span>
                        <h3 style='margin:6px 0 2px;'>{game['home_team']} <span style='color:#555;font-size:14px;'>vs</span> {game['away_team']}</h3>
                        <span style='color:#aaa;font-size:13px;'>🎯 Over 2.5 Goals · Predicted: <b style='color:#00ff00;'>{game['predicted_goals']:.2f}</b> goals</span>
                    </div>
                    <div style='text-align:center;margin-left:20px;'>
                        <div style='background:{badge_color};color:{text_color};border-radius:20px;padding:10px 18px;font-weight:bold;font-size:22px;'>{conf:.1f}%</div>
                        <div style='font-size:11px;color:#aaa;margin-top:4px;'>{game['confidence_level']}</div>
                    </div>
                </div>
                <div style='display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:8px;margin-top:14px;padding-top:12px;border-top:1px solid #2a2a4a;font-size:12px;'>
                    <div><span style='color:#888;'>Home xG</span><br><b style='color:#00ff00;'>{game['features']['home_goals']}</b></div>
                    <div><span style='color:#888;'>Away xG</span><br><b style='color:#00ff00;'>{game['features']['away_goals']}</b></div>
                    <div><span style='color:#888;'>Form</span><br><b style='color:#00ff00;'>{game['features']['form_factor']}</b></div>
                    <div><span style='color:#888;'>Fitness</span><br><b style='color:#00ff00;'>{game['features']['injury_impact']}</b></div>
                </div>
            </div>
            """, unsafe_allow_html=True)
        ...
        with btn_col:
            ...
            if is_logged:
                ...
                if st.button("📌 Log Pick", key=f"log_{game['id']}", use_container_width=True):
                    log_pick(game)
...
def render_results_tab():
    ...
    if st.button("🔄 Auto-Grade Picks", use_container_width=True):
        with st.spinner("Grading..."):
            n = auto_grade_picks()
        st.toast(f"✅ Graded {n} pick(s)!" if n else "Nothing to grade yet")
        st.rerun()

    picks = get_all_picks()
    if not picks:
        st.info("No picks yet — go to Predictions tab")
        return
    ...
    with c1: st.markdown(f"<div class='stat-box'><h3>{len(picks)}</h3><p>Total Picks</p></div>", unsafe_allow_html=True)
    with c2: st.markdown(f"<div class='stat-box'><h3 style='color:#00ff00;'>{len(won)}</h3><p>Won ✅</p></div>", unsafe_allow_html=True)
    with c3: st.markdown(f"<div class='stat-box'><h3 style='color:#ff4444;'>{len(lost)}</h3><p>Lost ❌</p></div>", unsafe_allow_html=True)
    with c4: st.markdown(f"<div class='stat-box'><h3>{len(pending)}</h3><p>Pending ⏳</p></div>", unsafe_allow_html=True)
    with c5:
        color = "#00ff00" if win_pct >= 60 else "#ffcc00" if win_pct >= 40 else "#ff4444"
        st.markdown(f"<div class='stat-box'><h3 style='color:{color};'>{win_pct:.1f}%</h3><p>Win Rate</p></div>", unsafe_allow_html=True)

    if graded > 0:
        st.markdown("---")
        ch1, ch2 = st.columns(2)
        with ch1:
            fig = go.Figure(go.Pie(labels=["Won ✅", "Lost ❌", "Pending ⏳"], values=[len(won), len(lost), len(pending)],
                                   marker_colors=["#00ff00", "#ff4444", "#555555"], hole=0.55, textinfo='label+percent'))
            fig.update_layout(title="Pick Outcomes", height=250, paper_bgcolor='rgba(0,0,0,0)', font=dict(color='#aaa'), showlegend=False)
    ...
    for p in picks:
        res = p['result']
        emoji = '✅ WON' if res == 'WON' else '❌ LOST' if res == 'LOST' else '⏳ Pending'
        border = "#00ff00" if res == 'WON' else "#ff4444" if res == 'LOST' else "#ffcc00"
        actual_str = f"{p['actual_goals']} goals" if p['actual_goals'] is not None else "awaiting result"
        row_col, del_col = st.columns([8, 1])
        with row_col:
            st.markdown(f"""
            <div style='border-left:4px solid {border};padding:10px 16px;margin:4px 0;border-radius:8px;background:rgba(255,255,255,0.03);'>
                <div style='display:flex;justify-content:space-between;align-items:center;'>
                    <div>
                        <b>{p['match_label']}</b> <span style='color:#888;font-size:12px;margin-left:10px;'>{p['league']}</span><br>
                        <span style='font-size:12px;color:#aaa;'>Confidence: <b style='color:#00ff00;'>{p['confidence']}%</b> · Predicted: <b>{p['pred_goals']}</b> · Actual: <b>{actual_str}</b></span>
                    </div>
...
def main():
    ...
    <div style='text-align:center;padding:10px 0 4px;'>
        ...
        <p style='color:#00ff00;font-size:12px;letter-spacing:2px;margin:0;'>OVER 2.5 GOALS · LIVE API-FOOTBALL · IMPROVED MODEL</p>
    </div>
    """, unsafe_allow_html=True)
    st.markdown("---")

    tab1, tab2 = st.tabs(["🎯 Predictions", "📊 Results"])
    with tab1: render_predictions_tab()
    with tab2: render_results_tab()
